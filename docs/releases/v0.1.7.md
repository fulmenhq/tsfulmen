...
```bash
echo "0.1.7" > VERSION
./bin/goneat version propagate  # Updates package.json
...
🔍 Verifying version consistency...
  ✅ Version consistency: 0.1.7
```

**Error Example** (if mismatch found):

```
❌ VERSION mismatch: package.json=0.1.7, exported=0.1.6
```
...
- `./bin/goneat` installation via `scripts/bootstrap-tools.ts` works correctly
- Downloads goneat v0.3.3 from GitHub releases to `./bin/`
- Makefile targets (`fmt`, `lint`, etc.) now work with local `./bin/goneat`
...
# Should pass with: ✅ Version consistency: 0.1.7
```
...
- ✅ Always run `make verify-artifacts` before publishing
- ✅ Use `scripts/version-sync.ts` for version bumps
- ✅ Verify VERSION constant matches package.json in built output
- ✅ Test bootstrap process works for new contributors
