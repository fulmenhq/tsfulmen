...
> **Breaking (v0.3.0 majors wave).** This wave raises the Node engine floor and migrates several major dependencies. See individual entries; the engine bump is finalized in the release-prep PR.
...
- **TypeScript (dev) 5.9.3 → 6.0.3** — clean compiler bump, no source changes. The `tsconfig.json` already sets all options explicitly (`target`/`module`/`lib` ES2022, `moduleResolution: bundler`, `esModuleInterop: true`, `strict`, `skipLibCheck`) and uses none of the options TS 6 removed (`outFile`, `classic`/`node10` resolution, `importsNotUsedAsValues`, etc.), so the default changes don't apply. Typecheck, full `tsup` `.d.ts` emit, and the `validate:exports`/`validate:types`/`validate:imports` consumer-surface suite all pass under 6.0.3.
