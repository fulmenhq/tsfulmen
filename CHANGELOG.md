...
- **commander 14.0.3 → 15.0.0** — commander 15 is ESM-only and requires `node>=22.12.0` (drives the engine-floor bump above). No source changes: tsfulmen is already ESM and all three CLIs (`foundry/signals`, `schema`, `telemetry/prometheus`) use the stable `import { Command }` API with only standard `--flag` / `--opt <val>` options — none use `--no-*` negated or paired options, so commander 15's negated-default parsing change does not apply. Validated by smoke-testing all three CLIs under v15 (help rendering, subcommands, `--json` parsing) plus the existing CLI-logic tests; the CLIs `program.parse()` on import, so commander-level arg-parse unit tests would need an `import.meta.main` guard + extracted `buildProgram()` (tracked as a testability follow-up, not part of this bump).
